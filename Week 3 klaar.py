...
# definitie van functie door alle variables op een rij te zetten en geen len functie omdat anders spaties meeteld en ik wil specefiek de letters
def tellen_letters(text):
    letters = "abcdefghijklmnopqrstvwxyz"
    alpha = {letter: 0 for letter in letters}
    
# letters klein maken van text met funcite text.lower met behulp van de functie en forloop
# in essentie loopt hij alles van alpha(bet) en maakt hij alles lowercase
    for x in text.lower():
        if x in letters:
            alpha[x] += 1
...
# maak hier nog een functie waarbij letters = waarde en dat dus opgeteld
def Letterstatus(aantal_letters):
    total_num_letters = sum(aantal_letters.values())


# gesorteerde variable, als dit niet staat dan teld hij automatisch de laagste waarde bovenaan wat ik niet wil, nu zie je de hoogste waarde bovenaan
# dit stuk overgenomen van python documentatie 2.7 wiki (begrijp dit deel niet dus gewoon overgenomen van wat ze daar zeiden over waardes sorteren)
    gesorteerd = sorted([(value, key) for (key, value) in aantal_letters.items()], reverse=True)


# percentage van waardes berekenen die gesorteerd zijn in 2 decimalen (:.2%)
    for value, letter in gesorteerd:
        percentage = 100 * value / total_num_letters
        print(f"{letter}: {value}\t({value / total_num_letters:.2%})")


# variables worden gelijkgesteld/gevoegd en uitgeprint
def main():
    aantal_letters = tellen_letters(text)
    Letterstatus(aantal_letters)
